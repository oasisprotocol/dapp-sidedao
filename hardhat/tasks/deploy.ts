...
function makeEnvUpdater(env:dotenv.DotenvParseOutput, filename?:string) {
  return async function updater(key:string, value:string) {
    ...
    console.log(line);
    if( filename ) {
      await fs.writeFile(filename, Object.entries(env).map(([k,v]) => `${k}=${v}`).join("\n") + "\n");
    }
  }
}
...
async function deploy_xchain(hre:HardhatRuntimeEnvironment, env: dotenv.DotenvParseOutput, setenv:ReturnType<typeof makeEnvUpdater>)
{
    const addr_HeaderCache = await deployContract(hre, await hre.ethers.getContractFactory('HeaderCache'), 'CONTRACT_XCHAIN_HEADERCACHE', env, setenv);

    const addr_AccountCache = await deployContract(hre, await hre.ethers.getContractFactory('AccountCache'), 'CONTRACT_XCHAIN_ACCOUNTCACHE', env, setenv, addr_HeaderCache);

    const addr_StorageProof = await deployContract(hre, await hre.ethers.getContractFactory('StorageProof'), 'CONTRACT_XCHAIN_STORAGEPROOF', env, setenv, addr_AccountCache);

    const addr_StorageProofACL = await deployContract(hre, await hre.ethers.getContractFactory('StorageProofACL'), 'CONTRACT_ACL_STORAGEPROOF', env, setenv, addr_StorageProof);

    return { addr_AccountCache, addr_HeaderCache, addr_StorageProof, addr_StorageProofACL };
}


async function deploy_acls(hre:HardhatRuntimeEnvironment, env:dotenv.DotenvParseOutput, setenv:ReturnType<typeof makeEnvUpdater>)
{
  const addr_AllowAllACL = await deployContract(hre, await hre.ethers.getContractFactory('AllowAllACL'), 'CONTRACT_ACL_ALLOWALL', env, setenv);

  const addr_VoterAllowListACL = await deployContract(hre, await hre.ethers.getContractFactory('VoterAllowListACL'), 'CONTRACT_ACL_VOTERALLOWLIST', env, setenv);

  const addr_TokenHolderACL = await deployContract(hre, await hre.ethers.getContractFactory('TokenHolderACL'), 'CONTRACT_ACL_TOKENHOLDER', env, setenv);
...
task('deploy')
  ...
  .setAction(async (args:DeployArgs, hre) => {
    ...
    const addr_TestToken = await deployContract(hre, await hre.ethers.getContractFactory('TestToken'), 'VITE_CONTRACT_TESTTOKEN', env, setenv);
    ...
    const addr_GaslessVoting = await deployContract(hre, await hre.ethers.getContractFactory('GaslessVoting'), 'CONTRACT_GASLESSVOTING', env, setenv);

    const addr_PollManager = await deployContract(hre, await hre.ethers.getContractFactory('PollManager'), 'CONTRACT_POLLMANAGER', env, setenv, addr_AllowAllACL, addr_GaslessVoting);
