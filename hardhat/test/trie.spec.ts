...
describe('Merkle Patricia Trie', () => {
    ...
    it('Accounts Tree', async () => {
        ...
        for( let i = 0; i < total; i += 1 )
        {
            ...
            const proofTree = await Trie.createFromProof(rpcEncodedAccountProof.map((_) => getBytes(_)), {useKeyHashing: true});
            ...
            const onChainAccountData = await accountCache.verifyAccount(proofTree.root(), hexlify(randAddress), contractEncodedProof);
