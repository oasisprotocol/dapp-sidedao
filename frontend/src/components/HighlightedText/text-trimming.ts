import { MatchInfo, findTextMatches, NO_MATCH, NormalizerOptions } from './text-matching'
...
export function trimAroundMatch(
  ...
  // do we have a match?
  const match = pattern ? (findTextMatches(corpus, [pattern], matchOptions)[0] ?? 'NO_MATCH') : NO_MATCH
